...
public final class DBBuffer implements DataHolder {
    ...
    public byte[] putAndGetData(DBObj dbobj) {
        dbobj.putBaseType(dbobj.getPrimaryKey(), this);
        Object[] relationKeys = dbobj.getRelationKeys();
        if(relationKeys!=null){
        	this.putByte((byte) relationKeys.length);
            for (int i = 0; i < relationKeys.length; i++) {
                dbobj.putBaseType(relationKeys[i], this);
            }
        }else{
        	this.putByte((byte)0);
        }
        boolean haveCid = dbobj.getCid() != null;
        this.putBoolean(haveCid);
        if (haveCid)
            this.putInt(dbobj.getCid());
        if (dbobj.getCreateDate() == null)
            dbobj.setCreateDate(new Date());

        this.putDate(dbobj.getCreateDate());
        if (dbobj.getModifyDate() == null)
            dbobj.setModifyDate(new Date());
        this.putDate(dbobj.getModifyDate());
        dbobj.writeBizData(this);
        return this.toBytes();
    }
    ...
    @Override
    public void putDBObj(DBObj dbobj) {
        DBBuffer dbBuffer = new DBBuffer();
        dbobj.writeToDBHolder(dbBuffer);
        putBytes(dbBuffer.toBytes());
    }
