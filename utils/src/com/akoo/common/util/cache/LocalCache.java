...
package com.akoo.common.util.cache;
...
public class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
   ...
   * exceeded.
   *
   * The page replacement algorithm's center structures are kept casually consistent with the map. The
   * ordering of writes to a segment is sequentially consistent. An update to the map and recording
   * of reads may not be immediately reflected on the algorithm's center structures. These structures
  ...
  static class Segment<K, V> extends ReentrantLock {
     ...
     * cause a concurrent read operation to see inconsistent center. This is made easier by the
    ...
    /**
     * 获取值，如果在内存中，将直接返回，不做失效验证
     * 张云峰
     * */
    @Nullable
    V getNotClear(Object key, int hash) {
      try {
        if (count != 0) { // read-volatile
          long now = map.ticker.read();
          ReferenceEntry<K, V> e = getEntry(key, hash);
  ...
  @Nullable
  public V getIfPresentNotClear(Object key) {
    int hash = hash(checkNotNull(key));
    V value = segmentFor(hash).getNotClear(key, hash);
    if (value == null) {
      globalStatsCounter.recordMisses(1);
    } else {
      globalStatsCounter.recordHits(1);
  ...
  public static class LocalManualCache<K, V> implements Cache<K, V>, Serializable {
     ...
     * */
    public V getIfPresentNotClear(Object key) {
      return localCache.getIfPresentNotClear(key);
    }
